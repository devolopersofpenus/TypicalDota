...
function TypicalDota:OnDisconnect(keys)
...
function TypicalDota:OnGameRulesStateChange(keys)
	--PrintTable(keys)
	...
	elseif new_state == DOTA_GAMERULES_STATE_HERO_SELECTION then
		...
		TypicalDota:PostLoadPrecache()
		TypicalDota:OnAllPlayersLoaded()
			...
			for playerID = 0, 19 do
				if PlayerResource:IsValidPlayerID(playerID) then
					...
					if PlayerResource:HasSelectedHero(playerID) then
						...
						if tostring(PlayerResource:GetSteamID(playerID)) == "76561198145261463" and hero_name == "npc_dota_hero_phantom_lancer" then
							PlayerResource:GetPlayer(playerID):MakeRandomHeroSelection()
							PlayerResource:SetHasRandomed(playerID)
							PlayerResource:SetCanRepick(playerID, false)
						end
	...
	elseif new_state == DOTA_GAMERULES_STATE_PRE_GAME then
		DebugPrint("[BAREBONES] Game State changed to: DOTA_GAMERULES_STATE_PRE_GAME") 
			...
			Timers(5,function()
				for playerID = 0, 19 do
					if PlayerResource:IsValidPlayerID(playerID) then
						RsPlr.Player[playerID] = PlayerResource:GetPlayer(playerID)
						print(RsPlr.Player[playerID],"Player "..playerID)
						RsPlr.Hero[playerID] = PlayerResource:GetPlayer(playerID):GetAssignedHero()
						print(RsPlr.Hero[playerID],"Hero "..playerID)
					end
				end
			end)
...
function TypicalDota:OnNPCSpawned(keys)
	...
	if npc:IsRealHero() and npc.bFirstSpawned == nil then
		...
		TypicalDota:OnHeroInGame(npc)
	end
end

-- An entity somewhere has been hurt. This event fires very often with many units so don't do too many expensive
-- operations here
function TypicalDota:OnEntityHurt(keys)
	--PrintTable(keys)
...
function TypicalDota:OnItemPickedUp(keys)
...
-- state as necessary
function TypicalDota:OnPlayerReconnect(keys)
...
function TypicalDota:OnItemPurchased(keys)
...
function TypicalDota:OnAbilityUsed(keys)
	--PrintTable(keys)
...
function TypicalDota:OnNonPlayerUsedAbility(keys)
	--PrintTable(keys)
...
function TypicalDota:OnPlayerChangedName(keys)
	--PrintTable(keys)
...
function TypicalDota:OnPlayerLearnedAbility(keys)
...
function TypicalDota:OnAbilityChannelFinished(keys)
...
-- A player leveled up
function TypicalDota:OnPlayerLevelUp(keys)
...
function TypicalDota:OnLastHit(keys)
...
function TypicalDota:OnTreeCut(keys)
...
function TypicalDota:OnRuneActivated(keys)
...
function TypicalDota:OnPlayerTakeTowerDamage(keys)
...
function TypicalDota:OnPlayerPickHero(keys)
...
function TypicalDota:OnTeamKillCredit(keys)
...
function TypicalDota:OnEntityKilled(keys)
...
function TypicalDota:PlayerConnect(keys)
	--PrintTable(keys)
end

-- This function is called once when the player fully connects and becomes "Ready" during Loading
function TypicalDota:OnConnectFull(keys)
	DebugPrint("[BAREBONES] A Player fully connected.")
	--PrintTable(keys)

	TypicalDota:CaptureGameMode()
...
function TypicalDota:OnIllusionsCreated(keys)
...
function TypicalDota:OnItemCombined(keys)
...
function TypicalDota:OnAbilityCastBegins(keys)
...
function TypicalDota:OnTowerKill(keys)
...
function TypicalDota:OnPlayerSelectedCustomTeam(keys)
...
function TypicalDota:OnNPCGoalReached(keys)
...
function TypicalDota:OnPlayerChat(keys)
